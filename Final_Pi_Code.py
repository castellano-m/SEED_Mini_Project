...
#    frame_markers = aruco.drawDetectedMarkers(image.copy(), corners, ids)
    ...
    # If the quadrant has changed, tell the arduino to move the motor
    global oldVal
    if quadrant != oldVal:
        arduino.write(quadrant.encode()) # Send the new quadrant
        oldVal = quadrant
        position = arduino.readline().decode('utf-8').rstrip() # Request the angular position from the arduino
        lcd.message = "Setpoint: %s\nPosition: %s" %(quadrant, position) # Print setpoint and angular position to the LCD screen
...
while True:
    ...
    if ids != None :   
        ...
        quadrant = '0' # Set the quadrant to zero if the camera does not detect an aruco marker
